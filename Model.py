...
from testPyBind import testPyBind
...
class Model: 
    ...
    def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False, dump=False):
        ...
        self.snapID = 0
        ##creating a WordbeamObject
        chars = str().join(self.charList)
        wordChars = open('../model/wordCharList.txt').read().splitlines()[0]
        corpus = open('../data/corpus.txt').read()
        self.wbs=WordBeamSearch(25,'Words',0.0,corpus.encode('utf8'), chars.encode('utf8'), wordChars.encode('utf8'))
    ...
    def setupCTC(self):
        ...
        if self.decoderType == DecoderType.BestPath:
            ...
            print("I am running wordBeam Search")
            ...
            # self.decoder = WordBeamSearch(tf.nn.softmax(self.ctcIn3dTBC, dim=2), 50, 'Words', 0.0,
            # 							  corpus.encode('utf8'), chars.encode('utf8'), wordChars.encode('utf8'))

            # self.decoder =WordBeamSearch(50, 'Words', 0.0, corpus.encode('utf8'), chars.encode('utf8'), wordChars.encode('utf8'))
            # self.feedMat=np.array(tf.nn.softmax(self.ctcIn3dTBC, dim=2))
            # self.res=testPyBind(self.feedMat,chars,wordChars,corpus)
            # print(self.res[0])
            # print(self.res[1])
        ...
        if self.decoderType == DecoderType.WordBeamSearch:
            ...
            for b in range(batchSize):
                for label in ctcOutput[b]:
                    if label==blank:
                        break
                    encodedLabelStrs[b].append(self.charList[label])
    ...
    def inferBatch(self, batch, calcProbability=False, probabilityOfGT=False):
        ...
        self.rnn_out_sm_numpy = self.sess.run(self.rnn_out_sm, feedDict)
        evalRes=self.wbs.compute(self.rnn_out_sm_numpy)
